  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="font-manrope font-bold text-2xl text-[#1a2820]">Report Side Effect</h1>
      ...
      {/* Form */}
      <div className="bg-white rounded-3xl border border-[#e0e8e2] p-6 space-y-6">
        
        {/* Medicine */}
        <div>
          <label className="flex items-center gap-2 text-xs font-semibold text-[#52615a] uppercase tracking-widest mb-3">
            <Pill className="w-3.5 h-3.5" /> Which Medicine? *
          </label>
          <div className="flex flex-wrap gap-2">
            {MEDICINES.map((m) => (
              <button key={m} onClick={() => setForm({ ...form, medicine: m })}
                className={`px-4 py-2 rounded-xl border text-sm font-medium transition-all ${ form.medicine === m ? "border-[#5E7464] bg-[#f0f8f2] text-[#42594A] " : "border-[#e0e8e2] text-[#52615a] hover:border-[#5E7464]/40" }`}>{m}</button>
            ))}
          </div>
        </div>
          ...
          <div className="flex flex-wrap gap-2">
            {SYMPTOMS.map((s) => (
              <button key={s} onClick={() => setForm({ ...form, symptom: s })}
                className={`px-4 py-2 rounded-xl border text-sm font-medium transition-all ${ form.symptom === s ? "border-[#5E7464] bg-[#f0f8f2] text-[#42594A] " : "border-[#e0e8e2] text-[#52615a] hover:border-[#5E7464]/40" }`}>{s}</button>
            ))}
          </div>
        </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            {SEVERITIES.map((s) => (
              <button key={s.value} onClick={() => setForm({ ...form, severity: s.value })}
                className={`p-4 rounded-2xl border-2 transition-all text-left ${ form.severity === s.value ? "" : "border-[#e0e8e2] hover:border-[#5E7464]/30" }`}
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="flex items-center gap-2 text-xs font-semibold text-[#52615a] uppercase tracking-widest mb-2">
              <Calendar className="w-3.5 h-3.5" /> When did it happen?
            </label>
            <input type="date" value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })}
              className="w-full px-4 py-3 rounded-xl border border-[#e0e8e2] bg-[#F8F8F4] text-[#1a2820] focus:border-[#5E7464] focus:ring-1 focus:ring-[#5E7464] outline-none text-sm" />
          </div>
          <div>
            <label className="flex items-center gap-2 text-xs font-semibold text-[#52615a] uppercase tracking-widest mb-2">
              <User className="w-3.5 h-3.5" /> Doctor Notified?
            </label>
            <div className="flex items-center gap-3 px-4 py-3.5 rounded-xl border border-[#e0e8e2] bg-[#F8F8F4] cursor-pointer"
              onClick={() => setForm({ ...form, doctorNotified: !form.doctorNotified })}>
              <div className={`w-10 h-5 rounded-full transition-colors relative ${form.doctorNotified ? "bg-[#5E7464]" : "bg-[#e0e8e2] "}`}>
              ...
              <span className="text-sm text-[#52615a]">{form.doctorNotified ? "Yes" : "No"}</span>
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <textarea placeholder="Describe what happened, any context, or other details…" rows={3}
            ...
            className="w-full px-4 py-3 rounded-xl border border-[#e0e8e2] bg-[#F8F8F4] text-[#1a2820] placeholder-[#9ab0a0] focus:border-[#5E7464] focus:ring-1 focus:ring-[#5E7464] outline-none text-sm resize-none" />
        </div>
