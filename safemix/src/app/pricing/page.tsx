...
function Cell({ val }: { val: boolean | string }) {
  if (typeof val === "string") return <span className="text-sm font-medium text-[#42594A] dark:text-[#9ab0a0]">{val}</span>;
  return val
      ...
      <main className="pt-[68px]">
        ...
        <section className="py-24 bg-[#F8F8F4] dark:bg-[#0f1410] text-center">
          <div className="max-w-3xl mx-auto px-6">
            <span className="inline-block px-4 py-1.5 rounded-full bg-[#5E7464]/10 text-[#42594A] dark:text-[#9ab0a0] text-xs font-bold tracking-widest uppercase mb-6">Simple Pricing</span>
              ...
              {plans.map((p) => (
                <div key={p.name}
                  className={`relative rounded-2xl p-8 flex flex-col transition-all duration-300 ${
                    p.highlight
                      ? "shadow-[0_20px_60px_rgba(94,116,100,0.25)] scale-[1.02]"
                      : "shadow-[0_4px_24px_rgba(0,0,0,0.06)] hover:shadow-[0_12px_40px_rgba(0,0,0,0.12)] hover:-translate-y-1"
                  }`}
                  style={p.highlight
                ...
                >
                  {p.badge && (
                    <div className={`absolute -top-3.5 left-1/2 -translate-x-1/2 px-4 py-1 rounded-full text-xs font-bold whitespace-nowrap ${
                      p.highlight ? "bg-[#3B82F6] text-white" : "bg-[#dceae0] text-[#42594A]"
                    }`}>
                      {p.badge}
                    </div>
                  )}
                  ...
                  <div className="mb-6">
                    <h3 className={`font-manrope font-bold text-xl mb-1 ${p.highlight ? "text-white" : "text-[#1a2820] dark:text-white"}`}>{p.name}</h3>
                    <p className={`text-sm mb-5 ${p.highlight ? "text-white/70" : "text-[#6b7b70] dark:text-[#7a9080]"}`}>{p.desc}</p>
                    <div className="flex items-end gap-2 mb-1">
                      <span className={`font-manrope font-bold text-5xl ${p.highlight ? "text-white" : "text-[#1a2820] dark:text-white"}`}>{p.price}</span>
                  ...
                  <ul className="space-y-3 mb-8 flex-1">
                    {p.features.map((f) => (
                      <li key={f} className="flex items-center gap-3">
                        <div className={`w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 ${p.highlight ? "bg-white/20" : "bg-[#dceae0] dark:bg-[#2a3430]"}`}>
                          <Check className={`w-3 h-3 ${p.highlight ? "text-white" : "text-[#5E7464]"}`} />
                        </div>
                        <span className={`text-sm ${p.highlight ? "text-white/85" : "text-[#52615a] dark:text-[#9ab0a0]"}`}>{f}</span>
                      </li>
                    ))}
                  </ul>

                  <Link href={p.href}
                    className={`block text-center py-3.5 rounded-xl text-sm font-semibold transition-all duration-200 hover:shadow-lg ${
                      p.highlight
                        ? "bg-white text-[#42594A] hover:bg-[#f0f5f1]"
        ...
        <section className="py-20 bg-[#F8F8F4] dark:bg-[#0f1410]">
          <div className="max-w-4xl mx-auto px-6">
            <h2 className="font-manrope font-bold text-3xl text-[#1a2820] dark:text-white text-center mb-12">Full Feature Comparison</h2>
            <div className="bg-white dark:bg-[#1e2820] rounded-2xl overflow-hidden shadow-[0_4px_24px_rgba(0,0,0,0.06)] border border-[#e0e8e2] dark:border-white/10">
              <table className="w-full">
                <thead>
                  <tr className="border-b border-[#e0e8e2] dark:border-white/10">
                    <th className="text-left px-6 py-4 text-sm font-semibold text-[#1a2820] dark:text-white">Feature</th>
                    {["Free","Pro","Enterprise"].map((h) => (
                      <th key={h} className="text-center px-4 py-4 text-sm font-semibold text-[#42594A] dark:text-[#9ab0a0]">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {tableFeatures.map((row, i) => (
                    <tr key={row.feature} className={`border-b border-[#f0f4f1] dark:border-white/5 ${i % 2 === 0 ? "" : "bg-[#f8faf8] dark:bg-[#1a2218]"}`}>
                      <td className="px-6 py-4 text-sm text-[#52615a] dark:text-[#9ab0a0]">{row.feature}</td>
